Общие положения:
* Система состоит из 2 компонентов: FPGA и R.Pi Zero 2w под управлением Raspberry PI OS (Debian)
...
Плата находится под управлением Raspberry Pi OS, подключена к FPGA по USB (для программирования и питания) и SPI, а также присоединена к энкодеру с функцией кнопки.
...
* __ivnd_testcard()__ - рисует настроечную таблицу (красная обводка, зеленые диагонали, синий крест через середины сторон). Возвращает 3 буфера
    ...
    * __ivnd_embed_number(value, frame_a, frame_n)__ - врисовывает маленькое 3-значное число в левый верхний угол для стробоскопа (счетчик FPS). Ничего не возвращает, но изменяет frame_a (always) и frame_n (never)
        * Применение:
        ...
        ```
В файле ivnd_fun методы:
* __ivnd_badapple(dev=IVNDDisplay)__ - демонстрация возможностей платформы (проигрывание GIF). Требуется библиотека pillow. dev - класс дисплея, нужно задать яркость и `render_static`
